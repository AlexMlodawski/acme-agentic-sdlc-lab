...
const portalPort = Number(process.env.PLAYWRIGHT_PORTAL_PORT ?? 3100);
...
export default defineConfig({
  ...
  reporter: [
    ["list"],
    ["html", { outputFolder: path.join(projectRoot, "playwright-report-built"), open: "never" }],
  ],
  outputDir: path.join(projectRoot, "test-results-built"),
  use: {
  ...
  webServer: [
    {
      command: `node scripts/e2e-server.mjs api-built ${apiPort}`,
      cwd: projectRoot,
    ...
    {
      command: `node scripts/e2e-server.mjs portal-built ${portalPort} ${apiPort}`,
      cwd: projectRoot,
  ...
  projects: [{ name: "chromium-built", use: { ...devices["Desktop Chrome"] } }],
});
